...
const convertMatchResultToImageSearchResult = (matchResult: MatchResult): ImageSearchResult => {
  ...
  return {
    item: {
      ...
      rarity: item.rarity ? item.rarity.charAt(0).toUpperCase() + item.rarity.slice(1) : 'Common',
      basePrice: Math.floor(Math.random() * 100000) + 10000,
      fleaMarketFee: Math.floor(Math.random() * 5000) + 1000,
      image: item.image || item.iconLink || `/api/items/${item.id}/image`,
      iconLink: item.iconLink || item.image || `/api/items/${item.id}/icon`,
      wikiLink: item.wikiLink || `https://escapefromtarkov.fandom.com/wiki/${item.name.replace(/ /g, '_')}`,
      types: item.types || [],
